...
//    @Scheduled(cron = "0 */1 * * * *", zone = "America/Sao_Paulo")
    public void startProcess() throws Exception {
         ...
         * */
        Instances trainInstances = leadAndFilter(false, "c1-train.arff", oneR_Detector1);
        Instances evaluationInstances = leadAndFilter(false, "c1-eval.arff", oneR_Detector1);
        Instances testInstances = leadAndFilter(false, "c1-test.arff", oneR_Detector1);
        ...
        detector.loadClassValues("c1-train.arff");
...
//                    System.out.println("Manter [" + i + "]:" + instances.attribute(i));
    ...
    public void learnWithAdvice(ConselorsDTO conselorsDTO) throws Exception {
        ...
        System.out.println("\t4- Creating new instance with labeled sample (Evaluate Instances)");
        Instance newEvaluateInstance = new DenseInstance(1.0, values);
        newEvaluateInstance.setDataset(evaluateInstances);

        System.out.println("\t5- Adding instance to evaluateInstances");
        trainInstances.add(newEvaluateInstance);

//        System.out.println("TAMANHO INSTANCIA DE TREINO: " + trainInstances.numAttributes());

        System.out.println("\t6- Retraining the classifiers");
        detector = trainingStage(detector, false);
        detector = evaluationStage(detector, false, true);
    }
}